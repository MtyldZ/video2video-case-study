...
- **Ranged playback.** Wherever a source video is shown next to a render (the Configure preview, the result comparison, the History drawer) it is delivered trimmed by Cloudinary (`so_<start>,eo_<end>`), so the player reports the clip's own duration and the source and result show the same seconds. Cloudinary derives the trimmed copy on first request (about a second) and caches it; `preload="none"` means nothing is fetched until play, so dragging the slider costs nothing.
