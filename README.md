...
- **Range preview.** The Configure step plays only the selected part of the source and loops it, by clamping the player rather than rendering a trimmed copy: instant while dragging and free. It streams the original Cloudinary URL, not the `q_auto` one, because a transformed copy reports a partial duration while Cloudinary is still deriving it, which breaks seeking.
