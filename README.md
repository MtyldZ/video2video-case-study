...
| Endpoint | Purpose |
|---|---|
| `POST /api/upload` | Copy a video uploaded to Uploadcare into Cloudinary |
| `POST /api/transform` | Create a record and start a Magic Hour job |
| `POST /api/webhook` | Receive Magic Hour's signed result, store it in Cloudinary and MongoDB |
| `GET /api/history` | This browser's transformations (also runs fallback polling) |
| `GET /api/credits` | Remaining Magic Hour credits |

Request/response examples and every status code: **[docs/api.md](docs/api.md)**.
...
- **Ranged playback.** Wherever a source video is shown next to a render (the Configure preview, the result comparison, the History drawer) the player is clamped to the selected range and loops it, so the source and the result show the same seconds. Clamping beats rendering a trimmed copy: instant while dragging and free. Ranged players stream the original Cloudinary URL, not the `q_auto` one, because a transformed copy reports a partial duration while Cloudinary is still deriving it, which breaks seeking.
